...
def preprocess_data(age, diastolicBP, BS, bodyTemp, heartRate):
    scaler_filename = "notebook/scaler_preg.pkl"
    scaler = pickle.load(open(scaler_filename, 'rb'))
    data = pd.DataFrame([[age, diastolicBP, BS, bodyTemp, heartRate]], columns=['Age', 'DiastolicBP', 'BS', 'BodyTemp', 'HeartRate'])
...
scaler_filename = "notebook/scaler.pkl"
scaler = pickle.load(open(scaler_filename, 'rb'))
def preprocess_data_fetal(BaselineValue, Accelerations, fetal_movement,
    ...
    data = pd.DataFrame([[BaselineValue, Accelerations, fetal_movement, uterine_contractions,
                          light_decelerations, severe_decelerations, prolongued_decelerations,
                          abnormal_short_term_variability, mean_value_of_short_term_variability,
                          percentage_of_time_with_abnormal_long_term_variability,
                          ...
                          histogram_tendency]], columns=columns)
...
if (selected == 'About us'):
    ...
    st.title("Welcome to Fetura")
    st.write("At Fetura, our mission is to revolutionize healthcare by offering innovative solutions through predictive analysis. "
    ...
    with col1:
        ...
        st.image("graphics/high-risk-care-preg-1.jpg", caption="Pregnancy Risk Prediction", use_column_width=True)
        st.image("graphics/high-risk-pregnancy-treatment-min.png", caption="Pregnancy Risk Prediction", use_column_width=True)
    with col2:
        ...
        st.image("graphics/images.jpg", caption="Fetal Health Prediction", use_column_width=True)
    ...
    with col1:
        if st.button('Predict Pregnancy Risk'):
            with warnings.catch_warnings():
                ...
                scaled_input1 = preprocess_data(float(age), float(diastolicBP), float(BS), float(bodyTemp), float(heartRate))
                predicted_risk = maternal_model.predict(scaled_input1)
                print("Scaled Input:", scaled_input1)
...
if (selected == 'Fetal Health Prediction'):
    ...
    with col1:
        if st.button('Predict Pregnancy Risk'):
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                scaled_input = preprocess_data_fetal(BaselineValue, Accelerations, fetal_movement,
                uterine_contractions, light_decelerations, severe_decelerations,
                prolongued_decelerations, abnormal_short_term_variability,
                mean_value_of_short_term_variability,
                percentage_of_time_with_abnormal_long_term_variability,
...
if (selected == "Dashboard"):
    api_key = "579b464db66ec23bdd000001f294fb17eace403d422877b16476a52f"
